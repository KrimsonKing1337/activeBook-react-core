import { Howl, type HowlOptions } from 'howler';
import type { AudioType, Timer } from '@types';
export type HowlWrapperOptions = {
    id: string;
    page?: number;
    src: HowlOptions['src'];
    loop?: HowlOptions['loop'];
    type?: AudioType;
    isRange?: boolean;
    playOnLoad?: boolean;
    relativeVolume?: number;
    screamer?: boolean;
    fadeOutWhenUnload?: boolean;
    delay?: number;
    stopBy?: number;
    onPlay?: () => void;
    onPause?: () => void;
    onStop?: () => void;
    onUnload?: () => void;
};
export declare class HowlWrapper {
    private static fadeDurationDefault;
    readonly howlInst: Howl;
    id: HowlWrapperOptions['id'];
    page: HowlWrapperOptions['page'];
    src: HowlOptions['src'];
    type: AudioType;
    isRange: boolean;
    playOnLoad: boolean;
    relativeVolume: number;
    fadeOutWhenUnload: boolean;
    delay: number;
    stopBy: number;
    isUnloading: boolean;
    isFading: boolean;
    timers: Record<string, Timer>;
    onPlay: () => void;
    onPause: () => void;
    onStop: () => void;
    onUnload: () => void;
    constructor({ id, page, src, loop, type, isRange, playOnLoad, relativeVolume, screamer, fadeOutWhenUnload, delay, stopBy, onPlay, onUnload, onPause, onStop, }: HowlWrapperOptions);
    volume(n: number): void;
    getVolume(): import("../../../store/volume/@types").State;
    getVolumeByType(): number;
    play(withFadeIn?: boolean): Promise<void>;
    pause(withFadeOut?: boolean): Promise<void>;
    stop(withFadeOut?: boolean): Promise<void>;
    unload(): Promise<void>;
    fade(from: number, to: number, dur: number): Promise<void>;
    fadeIn(): Promise<Howl>;
    fadeOut(): Promise<Howl>;
    playing(): boolean;
    state(): "unloaded" | "loading" | "loaded";
    waitTillTheEnd(): Promise<void>;
}
